...
class Measurements:
    ...
    def get_overal_waypoints (self, waypoints):
        robot_overal_waypoints = []
        human_overal_waypoints = []
        human_vis_overal_waypoints = []
        for pc in waypoints[0]:
            for w in pc:
                robot_overal_waypoints.append(w)
        for pc in waypoints[1]:
            for w in pc:
                human_overal_waypoints.append(w)
        for pc in waypoints[2]:
            for w in pc:
                human_vis_overal_waypoints.append(w)

        return [robot_overal_waypoints, human_overal_waypoints, human_vis_overal_waypoints]
    ...
    def log_grid_scenario (self, grid_log, file_name, walls_info, waypoints, planning_cycles, 
                           dir, rrt, optimal_distances, goals):
        overal_waypoints = self.get_overal_waypoints(waypoints)
        metrics = self.get_metrics(overal_waypoints, planning_cycles)
        #signal_log_r,signal_log_h = self.log_communication(waypoints[0],waypoints[1])
        #signal_log = [signal_log_r, signal_log_h]
        signal_log = [[], []]
        log = [grid_log, walls_info, waypoints, metrics, signal_log, rrt, optimal_distances, goals]
        path = os.path.dirname(os.path.realpath(__file__)) + "/" + dir + "/" + file_name
    ...
    def unsafe_func(self, waypoints):
        ...
        if len(unsafe) == 0: max_unsafe = 0
        else: max_unsafe = max(unsafe)
        return [unsafe, prox, max_unsafe]
