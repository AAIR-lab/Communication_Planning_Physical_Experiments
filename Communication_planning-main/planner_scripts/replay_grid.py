...
def load_data():
    ...
    file_dir = os.path.dirname(os.path.realpath(__file__)) + "/" + file_name
    map_name.set("")
    output.delete(1.0,END)
    if path.exists(file_dir):
        data = pk.load( open( file_dir, "rb" ) )
        debug = data[0]
        log_size = len (debug)
...
def show_overall():
    ...
    if flag:
        ...
        total_pc = len(waypoints [0])
        for j in range (total_pc):
            robot_x, robot_y = get_way_data(waypoints [0][j])
            human_sectionaized_waypoints = get_human_way_data(waypoints [1][j], waypoints [2][j])
            if j ==0: 
                lable_r, lable_h = "Robot", "Human"
            else: 
                lable_r, lable_h = "", ""
            plt.plot(robot_x, robot_y, color = '#135ff7', linewidth = 3, label = lable_r)

            for w_h in human_sectionaized_waypoints:
                human_x, human_y = get_way_data(w_h)
                plt.plot(human_x, human_y, color = '#e75a0e', linewidth = 3, label = lable_h)


            point = [robot_x[0], robot_y[0]]
            signal = waypoints[0][j][0][2]
            plt.text(point[0], point[1]+0.1, str(j + 1) + ': ' + str(signal), style = 'italic', fontsize = 9, color = "#fc38f2", label = 'Signal')   
            plt.scatter(point[0], point[1],color = "#135ff7")


            if waypoints [2][j][0]:
                point = waypoints [1][j][0]   
                plt.text(point[0], point[1]+0.1, str(j + 1), style = 'italic', fontsize = 9, color = "#e3221b", label = 'Signal')   
                plt.scatter(point[0], point[1],color = "#e75a0e")

        robot_goal, human_goal = data[7][0][0], data[7][1] 
        plt.scatter(robot_goal[0], robot_goal[1], color = '#135ff7', s = 100, marker = '*')
        plt.scatter(human_goal[0], human_goal[1], color = '#e75a0e', s = 100, marker = '*')
...
def draw_pc():
    ...
    if flag:
        ...
        for node in node_lists[scenario_index]:
            if node.parent:
                x0, x1, x2, x3 = zip(*node.path_x)
                plt.plot(x0, x1, linewidth = 0.7, color = '#cdcdcd')


        robot_x, robot_y = get_way_data(waypoints[0][scenario_index])
        human_sectionaized_waypoints = get_human_way_data(waypoints [1][scenario_index], waypoints [2][scenario_index])
        plt.plot(robot_x, robot_y, color = '#135ff7', linewidth = 3, label = 'Robot')
        
        for w_h in human_sectionaized_waypoints:
            if w_h == human_sectionaized_waypoints[0]: lable_h = "Human"
            else: lable_h = ""
            human_x, human_y = get_way_data(w_h)
            plt.plot(human_x, human_y, color = '#e75a0e', linewidth = 3, label = lable_h)
        ...
        point = [robot_x[0], robot_y[0]]
        signal = waypoints[0][scenario_index][0][2]
        plt.text(point[0], point[1]+0.1, str(scenario_index + 1) + ': ' + str(signal), style = 'italic', fontsize = 9, color = "#fc38f2", label = 'Signal')   
        plt.scatter(point[0], point[1],color = "#135ff7")
        
        if waypoints [2][scenario_index][0]:
            point = waypoints [1][scenario_index][0]   
            plt.text(point[0], point[1]+0.1, str(scenario_index + 1), style = 'italic', fontsize = 9, color = "#e3221b", label = 'Signal')   
            plt.scatter(point[0], point[1],color = "#e75a0e")

        
        plt.legend(prop={'size': 8})
        plt.title("Planning Cycle: " + str(scenario_index + 1), fontsize =8)
        plt.show()
    else:
        output.delete(1.0,END)
        output.insert(END, "No file is loaded!" + '\n')
...
def get_human_way_data (waypoints, human_visible):
    sectionized_waypoints = []
    temp = []
    for i in range (len (waypoints)):
        if human_visible[i]:
            temp.append(waypoints[i])
        else: 
            if temp != []: sectionized_waypoints.append(temp)
            temp = []
    if temp != []: sectionized_waypoints.append(temp)
    return sectionized_waypoints
...
show_pc = tkinter.Button(root,text = 'Show PC', command = draw_pc)
...
overview.grid(row=5,column=1)
show_pc.grid(row=4,column=1)
