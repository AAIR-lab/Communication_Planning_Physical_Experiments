...
class HighPlanner:
    ...
    def __init__(self, loc = (25,25), rotation = 0, r_radius = 0.5, fov_c = "#ffa538", line_w = 0.5,
                face_c = '#d56c11', outline_c = "b", body_s = 60, h_goal = [-7,1], show_stats = False, weights = [0.35, 0.35, 0.2, 0.1, 0.1]
    ...
    def give_plan(self, library, robot_loc, human_loc, human_dir, human_visible):
        self._tree = []
        if human_visible:
            self.update_geometry(human_loc, human_dir)
            #self._perseption = self._grid.get_pers (human_loc)
            self.update_human_pers(human_loc)
            index = 0
            ...
            for k in range(len(library)): 
                p = library[k]
                robot_tp_path_in_grid = self._grid.path_in_grid(p[3])
                c_robot = self.inf_multiplied(self.robot_cost(robot_tp_path_in_grid), self._weights[0])
                if c_robot < cost_min: 
                    cost_min = c_robot
                    min_index = k

            plan = library [min_index]
            c_robot = cost_min
            start = deepcopy(robot_loc)
            end = plan [0:2]
            signal = self._signals[-1]
            belief = []
            human_loc = self._human_goal
            cc = 0 #constant cost for all
            maze, path, _, _, human_synch_path = self._grid.give_path (human_loc, self._human_goal, 
                                                belief, False, start, plan, robot_tp_path_in_grid, self._delta_t, self._avg_velocity)

            new_node = Node(r_loc_c = robot_loc, r_loc_g = self._grid.where(robot_loc), r_plan_c = end, r_plan_g = self._grid.where(end), 
                                signal = signal, belief_g = belief, cost_r = cc, cost_h = cc, 
                                cost_traj = cc, signal_cost = cc, cost_head = cc,
                                human_loc_c = human_loc, human_loc_g = [0,0], n_human_dir = 0, 
                                n_index = min_index, human_path = path, n_maze = maze, h_synch_path = human_synch_path, 
                                r_synch_path = robot_tp_path_in_grid)
            self._tree.append (new_node)
...
class Visual:
    def __init__ (self, walls, x_lim = [-7.1,7.1], y_lim = (-2,6.7), back_c = '#5d5d5d', mon_size = (14, 14), face_c = '#d56c11', body_s = 60, rrt = None, human = None, robot = None, animation_mode = False):
        self._walls = walls
        self._xlim = x_lim
        self._ylim = y_lim
